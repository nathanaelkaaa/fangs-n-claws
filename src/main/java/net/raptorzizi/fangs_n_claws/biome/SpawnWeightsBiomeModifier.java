...
public record SpawnWeightsBiomeModifier() implements BiomeModifier {
    ...
    // YUNG's Cave Biomes compat — data-driven via biome keys, no hard dependency on the mod.
    private static ResourceKey<Biome> ycb(String path) {
        return ResourceKey.create(Registries.BIOME, new ResourceLocation("yungscavebiomes", path));
    }
