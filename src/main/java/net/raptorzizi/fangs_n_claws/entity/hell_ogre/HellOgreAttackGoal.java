...
public class HellOgreAttackGoal extends Goal {

    // Variables

    private final HellOgreEntity ogre;
    ...
    static final double         FIRE_BREATH_MAX_RANGE = 10.0;
    private static final int    FIRE_BREATH_COOLDOWN = 180;
    ...
    public HellOgreAttackGoal(HellOgreEntity ogre) {
        this.ogre = ogre;
    ...
    public void tick() {
        ...
        if (ogre.isAttacking() || ogre.isSlamming() || ogre.isFireBreathActive()) {
            ogre.setRunning(false);
            ogre.getNavigation().stop();
        ...
        if (ogre.getNavigation().isStuck()) {
            stuckTick++;
            if (ogre.onGround()) ogre.getJumpControl().jump();
            if (stuckTick % 20 == 0) ogre.getNavigation().moveTo(target, CHASE_SPEED);
        } else {
            stuckTick = 0;
        }
        ...
        if (slamCooldown <= 0) {
            ...
            boolean stuckSlam     = stuckTick >= STUCK_TICK_THRESHOLD && !ogre.hasLineOfSight(target);

            if (gapCloserSlam || stuckSlam) {
                Vec3 toTarget = target.position().subtract(ogre.position());
                Vec3 dir = toTarget.lengthSqr() > 0 ? toTarget.normalize() : ogre.getLookAngle();
        ...
        if (fireBreathCooldown <= 0) {
            double dist = ogre.distanceTo(target);
            if (dist > MAX_ATTACK_RANGE && dist <= FIRE_BREATH_MAX_RANGE
                    && ogre.hasLineOfSight(target)) {
                ogre.triggerFireBreath();
